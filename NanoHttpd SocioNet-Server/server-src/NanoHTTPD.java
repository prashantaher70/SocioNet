...
public abstract class NanoHTTPD {
    ...
    public static class DefaultTempFileManager implements TempFileManager {
        ...
        private final String tmpdirRand;
		
        private final List<TempFile> tempFiles;

        public DefaultTempFileManager() {
            tmpdir =".\\profilepics\\";//System.getProperty("java.io.tmpdir");
            tmpdirRand =".\\temp\\";//System.getProperty("java.io.tmpdir");
            tempFiles = new ArrayList<TempFile>();
        }

        @Override
        public TempFile createTempFile(String filename) throws Exception {
            DefaultTempFile tempFile = new DefaultTempFile(tmpdir,tmpdirRand,filename);
    ...
    public static class DefaultTempFile implements TempFile {
        ...
        public DefaultTempFile(String tempdir,String tmpdirRand,String filename) throws IOException {
            filename=filename.replace("\"","");
			if(filename.equals(""))
			{
				isTemp=true;
				file=File.createTempFile("NanoHTTPD-","",new File(tmpdirRand));
			}
			else
			{
				isTemp=false;
        ...
        public String getName() {
			...
			if(isTemp)
			{
				return ".\\temp\\"+file.getName();
			}
			return ".\\profilepics\\"+file.getName();
        }
    }
    ...
    protected class HTTPSession {
                ...
                while (mpline != null) {
                    ...
                    if (mpline != null) {
                        ...
                        if (item.get("content-type") == null) {
							...
							String path = saveTmpFile(fbuf, offset, bpositions[boundarycount - 1] - offset - 4,disposition.get("filename"));
         ...
         */
        private String saveTmpFile(ByteBuffer  b, int offset, int len,String filename) {
            String path = "";
			
            if (len > 0) {
                FileOutputStream fileOutputStream = null;
                try {
                    TempFile tempFile = tempFileManager.createTempFile(filename);
